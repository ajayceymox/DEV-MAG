 ...
 */
namespace Zazmic\AmazonMCF\Model\ResourceModel\MoreSkuMapping\Grid;

use Magento\Framework\Api\Search\SearchResultInterface as SearchInterface;
use Magento\Framework\Search\AggregationInterface;
use Zazmic\AmazonMCF\Model\ResourceModel\MoreSkuMapping\Collection as GirdCollection;
...
class Collection extends GirdCollection implements SearchInterface
{
    ...
    public function __construct(
        ...
        Request $request,
        GirdCollection $customCollection,
        $mainTable,
        $eventPrefix,
        $eventObject,
        $resourceModel,
    ...
    protected function _initSelect()
    {
          ...
          $customData =  $this->customCollection->addFieldToSelect('updated_at')
        ->setOrder('updated_at', 'desc');
         $customData->getSelect()->limit(1);
         $updateTime = $customData->getData();
        if (isset($updateTime[0])) {
            $updateTime = $updateTime[0]['updated_at'];
        } else {
            $updateTime = '';
        }
        ...
        if ($this->request->getParam('website')) {
            $joinCondition = ' and 
            msm.website = '.$this->request->getParam('website');

        }
        $this->addFieldToFilter('main_table.created_at', ['gt' => $updateTime]);
        ...
        ->joinLeft('mcf_sku_mapping as msm', 'main_table.entity_id=msm.product_id '. $joinCondition, [
            ...
            'status' => 'IFNULL(msm.status,0) AS msm.status',
      ...
      //  $this->collection->addAttributeToFilter('main_table.created_at', ['gt' =>'2022-01-24 11:23:01']);
